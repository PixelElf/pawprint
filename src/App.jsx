...
function AuthScreen({onLogin}){
  ...
  const verifyOtp=()=>{
    ...
    setError('');
    const userId='user-'+phone.replace(/[^0-9]/g,'');
    onLogin({id:userId,phone:phone.trim(),name:name.trim()||'PawPrint User',address:address.trim()||null});
  };
          ...
          {step==='phone'?(
            ...
            <>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                ...
                {error&&<p style={{margin:0,fontSize:12,color:C.danger,fontFamily:F}}>{error}</p>}
                <Btn onClick={verifyOtp} style={{width:'100%',background:'linear-gradient(135deg,'+C.primary+','+C.primaryDark+')',color:'white',fontSize:16,padding:'14px 24px',borderRadius:12}}>
                  {mode==='signup'?'Create Account & Sign In':'Log In'}
                </Btn>
...
function AccountSettings({user,onUpdate,onDelete,onBack}){
  ...
  const handleSave=()=>{
    ...
    if(phoneDigits.length<8){alert('Enter a valid WhatsApp number');return;}
    const isPhoneChange=newPhone.trim()!==user.phone;
    onUpdate({...user,name:name.trim(),address:address.trim()||null,phone:newPhone.trim()});
    setSaved(true);
    if(isPhoneChange) setPhoneChanged(true);
    setTimeout(()=>{setSaved(false);setPhoneChanged(false);},3000);
  };
    ...
    <div style={{animation:'fadeIn 0.3s ease'}}>
      ...
      <div style={{background:C.card,borderRadius:20,padding:'28px 24px',border:'1.5px solid '+C.border,boxShadow:'0 4px 16px rgba(0,0,0,0.06)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}>
          <div style={{width:44,height:44,borderRadius:22,background:C.surface,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <GearIcon size={22}/>
          </div>
          <div>
            <h2 style={{margin:0,fontFamily:FD,fontSize:22,color:C.text}}>Account Settings</h2>
            <p style={{margin:0,fontFamily:F,fontSize:12,color:C.textMuted}}>Manage your profile and preferences</p>
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{borderTop:'1px solid '+C.border,paddingTop:16}}>
            <Input label="WhatsApp Number" value={newPhone} onChange={setNewPhone} placeholder="e.g. +971 50 123 4567" required type="tel" note="Changing this updates your contact number on all your pet listings. In production, you'll need to re-verify."/>
          </div>

          <Btn onClick={handleSave} style={{width:'100%',padding:'14px 24px',borderRadius:12,background:'linear-gradient(135deg,'+C.primary+','+C.primaryDark+')',color:'white',fontSize:16,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
            Save Changes
          </Btn>
          ...
          {saved&&(
            <div style={{padding:'10px 14px',background:'#ecfdf5',borderRadius:10,border:'1px solid #a7f3d0',fontFamily:F,fontSize:13,color:'#065f46',textAlign:'center'}}>
              ✅ {phoneChanged?'Settings saved. In production, your new number would require SMS verification.':'Settings saved successfully!'}
            </div>
          )}
          ...
          <div style={{borderTop:'1px solid '+C.border,paddingTop:16,marginTop:8}}>
            ...
            <p style={{margin:'0 0 12px',fontFamily:F,fontSize:12,color:C.textMuted}}>Deleting your account removes all your pet listings permanently. This cannot be undone.</p>
            {!confirmDelete?(
              <Btn onClick={()=>setConfirmDelete(true)} style={{background:'transparent',color:C.danger,border:'1.5px solid '+C.danger,width:'100%',justifyContent:'center'}}>
                Delete My Account
              </Btn>
            ):(
              <div style={{padding:14,background:'#fef2f2',borderRadius:12,display:'flex',flexDirection:'column',gap:10}}>
                <p style={{margin:0,fontFamily:F,fontSize:13,color:C.danger,fontWeight:600}}>Are you absolutely sure? All your pets will be removed.</p>
                <div style={{display:'flex',gap:10}}>
                  <Btn onClick={onDelete} style={{flex:1,background:C.danger,color:'white',justifyContent:'center'}}>Yes, Delete Everything</Btn>
    ...
    try{
      if(initial?.id){
        const{error}=await supabase.from('pets').update({
          type,breed,name:name.trim(),description:description.trim()||null,photo_urls:photos,
        }).eq('id',initial.id);
        if(error)throw error;
      }else{
        const{error}=await supabase.from('pets').insert({
          type,breed,name:name.trim(),description:description.trim()||null,
          owner_id:user.id,whatsapp:user.phone,photo_urls:photos,
        });
        if(error)throw error;
      }
      onSubmit();
    ...
    <div style={{animation:'fadeIn 0.3s ease'}}>
      ...
      <div style={{background:C.card,borderRadius:20,padding:'28px 24px',border:'1.5px solid '+C.border,boxShadow:'0 4px 16px rgba(0,0,0,0.06)'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div>
              ...
              <div style={{display:'flex',gap:8}}>
                {['cat','dog'].map(t=>(
                  <button type="button" key={t} onClick={()=>{setType(t);setBreed('');}} style={{
                    flex:1,padding:'10px 8px',borderRadius:10,cursor:'pointer',border:'2px solid '+(type===t?C.primary:C.border),
                    background:type===t?'#fef3c7':'white',fontFamily:F,fontSize:14,fontWeight:600,
          ...
          <Btn onClick={handleSubmit} disabled={submitting} style={{
            width:'100%',padding:'14px 24px',borderRadius:12,
            background:submitting?C.textMuted:'linear-gradient(135deg,'+C.primary+','+C.primaryDark+')',
            color:'white',fontSize:16,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:8,
            boxShadow:'0 4px 12px rgba(180,83,9,0.25)',marginTop:4
          }}>
...
function PetDetail({pet,isOwner,onBack,onDelete,onEdit}){
  ...
  const waLink=pet.whatsapp_link
    ?pet.whatsapp_link+'?text='+encodeURIComponent('Hi! I\'m reaching out about your pet "'+pet.name+'" from PawPrint Villanova.')
    :'#';
        ...
        <div style={{padding:'20px 24px 24px'}}>
          ...
          {isOwner&&(
            <div style={{marginTop:20,padding:16,background:'#fef3c7',borderRadius:14,border:'1px solid #fde68a'}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                <Btn onClick={onEdit} style={{background:C.primary,color:'white',display:'flex',alignItems:'center',gap:6,flex:1,justifyContent:'center'}}>
...
export default function App(){
  ...
  const[view,setView]=useState('browse'); // browse|register|edit|detail|settings
  const[selectedPet,setSelectedPet]=useState(null);
  const[editPet,setEditPet]=useState(null);
  const[tab,setTab]=useState('all');
  const[search,setSearch]=useState('');
  const[page,setPage]=useState('browse'); // browse|myPets
  ...
  const handleDeleteAccount=async()=>{
    try{
      const{error}=await supabase.from('pets').delete().eq('owner_id',user.id);
      if(error)throw error;
      setUser(null);
    ...
    <div>
      ...
      <div style={{minHeight:'100vh',background:C.bg}}>
        ...
        <div style={{background:'linear-gradient(135deg,'+C.primaryDark+' 0%,'+C.primary+' 50%,#d97706 100%)',padding:'20px 20px 18px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{position:'relative',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <button type="button" onClick={()=>{setView('settings');setPage('browse');}} style={{background:'rgba(255,255,255,0.15)',border:'none',borderRadius:8,padding:'6px 8px',color:'white',cursor:'pointer',display:'flex',alignItems:'center'}}>
                <GearIcon size={16}/>
              </button>
              <button type="button" onClick={()=>setUser(null)} style={{background:'rgba(255,255,255,0.15)',border:'none',borderRadius:8,padding:'6px 12px',color:'white',fontFamily:F,fontSize:11,fontWeight:600,cursor:'pointer'}}>Sign Out</button>
            </div>
          </div>
        </div>
        ...
        <div style={{maxWidth:600,margin:'0 auto',padding:'16px 16px 80px'}}>
          {/* Settings */}
          {view==='settings'&&(
            <AccountSettings user={user} onUpdate={handleUpdateUser} onDelete={handleDeleteAccount} onBack={()=>setView('browse')}/>
          )}
              ...
              <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap'}}>
                <div style={{flex:1,minWidth:160,position:'relative'}}>
                  <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or breed..."
                    style={{width:'100%',padding:'10px 14px 10px 36px',borderRadius:12,border:'1.5px solid '+C.border,fontSize:14,fontFamily:F,background:'white',outline:'none',boxSizing:'border-box'}}/>
                    ...
                    <div key={pet.id} style={{background:C.card,borderRadius:14,border:'1.5px solid '+C.border,padding:14,display:'flex',gap:14,alignItems:'center',cursor:'pointer'}}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:'flex',gap:8,marginTop:6}}>
                          <button type="button" onClick={e=>{e.stopPropagation();setEditPet(pet);setView('edit');}} style={{padding:'4px 12px',borderRadius:6,border:'1px solid '+C.border,background:'white',fontFamily:F,fontSize:11,fontWeight:600,color:C.primary,cursor:'pointer',display:'flex',alignItems:'center',gap:4}}>
                            <EditIcon/> Edit
                          </button>
                        </div>
                      </div>
