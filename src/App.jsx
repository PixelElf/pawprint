...
const V={navy:'#1a1f3a',navyLight:'#252b4a',gold:'#c9a84c',goldLight:'#e8d48b',goldDark:'#a88a2e',cream:'#faf8f3',cardBg:'#ffffff',text:'#1a1f3a',textLight:'#6b7194',border:'#e8e4dc',accent:'#2d8a6e',danger:'#c0392b',whatsapp:'#25D366',surface:'#f2efe8'};
const F="'Nunito Sans', sans-serif";
const FD="'Cormorant Garamond', serif";

const LIFESTYLE_OPTIONS=[{value:'indoor',label:'🏠 Indoor'},{value:'outdoor',label:'🌳 Outdoor'},{value:'indoor_outdoor',label:'🏠🌳 Both'}];
const lifestyleLabel=val=>{const o=LIFESTYLE_OPTIONS.find(x=>x.value===val);return o?o.label:'🏠🌳 Both';};
...
const MailIcon=({size=28})=>(<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>);

// ── Shared UI ──
function Input({label,value,onChange,placeholder,required,type,maxLength,note}){
  return(<div>{label&&<label style={{display:'block',marginBottom:5,fontWeight:700,color:V.navy,fontFamily:F,fontSize:13,textTransform:'uppercase',letterSpacing:0.5}}>{label} {required&&<span style={{color:V.danger}}>*</span>}</label>}
    <input type={type||'text'} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} maxLength={maxLength} style={{width:'100%',padding:'12px 16px',borderRadius:8,border:'1.5px solid '+V.border,fontSize:15,fontFamily:F,background:'white',color:V.text,outline:'none',boxSizing:'border-box',transition:'border-color 0.2s'}} onFocus={e=>{e.target.style.borderColor=V.gold}} onBlur={e=>{e.target.style.borderColor=V.border}}/>
    {note&&<p style={{margin:'4px 0 0',fontSize:11,color:V.textLight,fontFamily:F}}>{note}</p>}</div>);
}
function TextArea({label,value,onChange,placeholder,maxLength}){
  return(<div>{label&&<label style={{display:'block',marginBottom:5,fontWeight:700,color:V.navy,fontFamily:F,fontSize:13,textTransform:'uppercase',letterSpacing:0.5}}>{label}</label>}
    <textarea value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} maxLength={maxLength} rows={3} style={{width:'100%',padding:'12px 16px',borderRadius:8,border:'1.5px solid '+V.border,fontSize:15,fontFamily:F,background:'white',color:V.text,outline:'none',boxSizing:'border-box',resize:'vertical',minHeight:80}} onFocus={e=>{e.target.style.borderColor=V.gold}} onBlur={e=>{e.target.style.borderColor=V.border}}/>
    {maxLength&&<div style={{textAlign:'right',fontSize:11,color:V.textLight,marginTop:2}}>{value.length}/{maxLength}</div>}</div>);
}
function BreedSelect({label,value,onChange,options,required,placeholder,disabled}){
  ...
  return(<div ref={ref} style={{position:'relative'}}>{label&&<label style={{display:'block',marginBottom:5,fontWeight:700,color:V.navy,fontFamily:F,fontSize:13,textTransform:'uppercase',letterSpacing:0.5}}>{label} {required&&<span style={{color:V.danger}}>*</span>}</label>}
    <button type="button" onClick={()=>!disabled&&setOpen(!open)} disabled={disabled} style={{width:'100%',padding:'12px 16px',borderRadius:8,border:'1.5px solid '+V.border,fontSize:15,fontFamily:F,background:disabled?V.surface:'white',color:value?V.text:V.textLight,textAlign:'left',cursor:disabled?'default':'pointer',boxSizing:'border-box',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{value||placeholder}</span><span style={{fontSize:10,opacity:0.4}}>▼</span></button>
...
function PhotoUploader({photos,setPhotos}){
  ...
  return(<div><label style={{display:'block',marginBottom:6,fontWeight:700,color:V.navy,fontFamily:F,fontSize:13,textTransform:'uppercase',letterSpacing:0.5}}>Photos ({photos.length}/3) <span style={{color:V.danger}}>*</span></label>
    <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>{photos.map((p,i)=>(<div key={i} style={{position:'relative',width:100,height:100,borderRadius:10,overflow:'hidden',border:'2px solid '+V.border,boxShadow:'0 2px 8px rgba(0,0,0,0.08)'}}><img src={p} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/><button type="button" onClick={()=>setPhotos(prev=>prev.filter((_,j)=>j!==i))} style={{position:'absolute',top:4,right:4,width:22,height:22,borderRadius:'50%',background:V.danger,color:'white',border:'none',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center'}}>×</button></div>))}
    ...
    <input ref={fileRef} type="file" accept="image/*" multiple onChange={handleAdd} style={{display:'none'}}/></div>);
}
function LifestyleSelector({value,onChange}){
  return(<div><label style={{display:'block',marginBottom:5,fontWeight:700,color:V.navy,fontFamily:F,fontSize:13,textTransform:'uppercase',letterSpacing:0.5}}>Lifestyle <span style={{color:V.danger}}>*</span></label>
    <div style={{display:'flex',gap:8}}>{LIFESTYLE_OPTIONS.map(opt=>(<button type="button" key={opt.value} onClick={()=>onChange(opt.value)} style={{flex:1,padding:'10px 6px',borderRadius:8,cursor:'pointer',border:'2px solid '+(value===opt.value?V.gold:V.border),background:value===opt.value?'rgba(201,168,76,0.08)':'white',fontFamily:F,fontSize:12,fontWeight:700,color:value===opt.value?V.navy:V.textLight,display:'flex',flexDirection:'column',alignItems:'center',gap:2}}><span style={{fontSize:16}}>{opt.value==='indoor'?'🏠':opt.value==='outdoor'?'🌳':'🏠🌳'}</span><span>{opt.value==='indoor'?'Indoor':opt.value==='outdoor'?'Outdoor':'Both'}</span></button>))}</div>
    <p style={{margin:'4px 0 0',fontSize:11,color:V.textLight,fontFamily:F}}>{value==='indoor'?'Stays inside — important if spotted outdoors':value==='outdoor'?'Roams freely outside':'Moves between indoors and outdoors'}</p></div>);
}

// ═══════════════════════════════════════
// AUTH — Email OTP (Sign Up / Log In)
// ═══════════════════════════════════════
function AuthScreen({onLogin}){
  const[mode,setMode]=useState('signup');const[step,setStep]=useState('form');const[email,setEmail]=useState('');const[phone,setPhone]=useState('');const[otp,setOtp]=useState('');const[name,setName]=useState('');const[address,setAddress]=useState('');const[error,setError]=useState('');const[sending,setSending]=useState(false);

  const sendOtp=async()=>{
    if(!email.trim()||!email.includes('@')){setError('Enter a valid email address');return;}
    if(mode==='signup'){
      if(!name.trim()){setError('Enter your name');return;}
      if(phone.replace(/[^0-9]/g,'').length<8){setError('Enter a valid WhatsApp number with country code');return;}
    }
    setError('');setSending(true);
    try{const{error:e}=await supabase.auth.signInWithOtp({email:email.trim().toLowerCase()});if(e)throw e;setStep('otp');}
  ...
  const verifyOtp=async()=>{
    ...
    setError('');setSending(true);
    try{
      const{data,error:e}=await supabase.auth.verifyOtp({email:email.trim().toLowerCase(),token:otp,type:'email'});if(e)throw e;
      let p=phone.trim();if(p&&!p.startsWith('+'))p='+'+p;p=p.replace(/\s/g,'');
      if(name.trim()||p)await supabase.auth.updateUser({data:{display_name:name.trim()||undefined,whatsapp:p||undefined,address:address.trim()||undefined}});
      const u=data.session?.user;
      if(u)onLogin({id:u.id,email:u.email||email.trim(),phone:u.user_metadata?.whatsapp||p||'',name:u.user_metadata?.display_name||name.trim()||'PawPrint User',address:u.user_metadata?.address||address.trim()||null});
    ...
    <div style={{flex:1,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'30px 20px',background:V.cream}}>
      <div style={{width:'100%',maxWidth:420,background:V.cardBg,borderRadius:16,padding:'32px 28px',border:'1px solid '+V.border,boxShadow:'0 8px 30px rgba(26,31,58,0.08)'}}>
        <div style={{display:'flex',background:V.surface,borderRadius:10,padding:3,marginBottom:24}}>
          {[{k:'signup',l:'Sign Up'},{k:'login',l:'Log In'}].map(m=>(<button type="button" key={m.k} onClick={()=>{setMode(m.k);setStep('form');setError('');setOtp('');}} style={{flex:1,padding:'10px',borderRadius:8,border:'none',cursor:'pointer',fontFamily:F,fontSize:14,fontWeight:700,background:mode===m.k?'white':'transparent',color:mode===m.k?V.navy:V.textLight,boxShadow:mode===m.k?'0 2px 8px rgba(0,0,0,0.06)':'none',transition:'all 0.2s'}}>{m.l}</button>))}
        </div>
        {step==='form'?(<>
          <div style={{textAlign:'center',marginBottom:24}}>
            <div style={{width:60,height:60,borderRadius:30,background:V.navy,display:'inline-flex',alignItems:'center',justifyContent:'center',marginBottom:14,boxShadow:'0 4px 14px rgba(26,31,58,0.3)'}}><MailIcon size={28}/></div>
            ...
            <p style={{margin:0,fontFamily:F,fontSize:13,color:V.textLight}}>{mode==='signup'?'Sign up — we\'ll email you a verification code':'Log in with your email address'}</p>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            {mode==='signup'&&(<>
              <Input label="Your Name" value={name} onChange={setName} placeholder="e.g. Ahmed, Sarah" required/>
              <Input label="WhatsApp Number" value={phone} onChange={setPhone} placeholder="e.g. +971 50 123 4567" required type="tel" note="Required — this is how other residents can contact you about your pets via the anonymous WhatsApp button. Your number is never displayed."/>
              <Input label="Address" value={address} onChange={setAddress} placeholder="Optional — villa/unit number" note="Optional. Never shown publicly. Helps community admins if needed."/>
            </>)}
            <Input label="Email Address" value={email} onChange={setEmail} placeholder="e.g. name@email.com" required type="email" note={mode==='signup'?"We'll send a 6-digit code to verify your email.":"Enter the email you signed up with."}/>
            {error&&<p style={{margin:0,fontSize:12,color:V.danger,fontFamily:F}}>{error}</p>}
            <button type="button" onClick={sendOtp} disabled={sending} style={{width:'100%',padding:'14px',borderRadius:10,border:'none',background:'linear-gradient(135deg,'+V.navy+','+V.navyLight+')',color:'white',fontSize:16,fontFamily:F,fontWeight:700,cursor:sending?'wait':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,opacity:sending?0.7:1,boxShadow:'0 4px 14px rgba(26,31,58,0.25)'}}>
              {sending?'Sending code...':(mode==='signup'?'Send Verification Code':'Send Login Code')}
            </button>
          </div>
          <div style={{marginTop:18,padding:'12px 14px',background:'rgba(45,138,110,0.06)',borderRadius:10,border:'1px solid rgba(45,138,110,0.15)',display:'flex',gap:8,alignItems:'flex-start'}}><span style={{color:V.accent,marginTop:1,flexShrink:0}}><ShieldIcon size={14}/></span><p style={{margin:0,fontFamily:F,fontSize:11,color:V.accent,lineHeight:1.5}}>Your WhatsApp number is never displayed publicly. When someone taps "Contact Owner" on your pet's listing, they're taken to WhatsApp with a pre-filled message — they don't see your number until you respond.</p></div>
        </>):(<>
          <div style={{textAlign:'center',marginBottom:24}}>
            <h2 style={{margin:'0 0 4px',fontFamily:FD,fontSize:24,color:V.navy,fontWeight:600}}>Check Your Email</h2>
            <p style={{margin:0,fontFamily:F,fontSize:13,color:V.textLight}}>We sent a code to <strong style={{color:V.navy}}>{email}</strong></p>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            <button type="button" onClick={()=>{setStep('form');setOtp('');setError('');}} style={{background:'none',border:'none',color:V.gold,fontFamily:F,fontSize:13,cursor:'pointer',fontWeight:700}}>← Change email</button>
            <p style={{margin:0,fontSize:11,color:V.textLight,fontFamily:F,textAlign:'center'}}>Check your spam/junk folder if you don't see the email within a minute.</p>
          </div>
        </>)}
      </div>
    </div>
...
function AccountSettings({user,onUpdate,onDelete,onBack}){
  const[name,setName]=useState(user.name||'');const[address,setAddress]=useState(user.address||'');const[phone,setPhone]=useState(user.phone||'');const[saved,setSaved]=useState(false);const[saving,setSaving]=useState(false);const[confirmDel,setConfirmDel]=useState(false);
  const handleSave=async()=>{if(!name.trim()){alert('Name is required');return;}let p=phone.trim();if(p&&!p.startsWith('+'))p='+'+p;p=p.replace(/\s/g,'');setSaving(true);try{await supabase.auth.updateUser({data:{display_name:name.trim(),whatsapp:p||undefined,address:address.trim()||null}});onUpdate({...user,name:name.trim(),phone:p,address:address.trim()||null});setSaved(true);setTimeout(()=>setSaved(false),3000);}catch(err){alert('Failed: '+err.message);}setSaving(false);};
  return(<div style={{animation:'fadeIn 0.3s ease'}}>
    ...
    <div style={{background:V.cardBg,borderRadius:16,padding:'32px 28px',border:'1px solid '+V.border,boxShadow:'0 8px 30px rgba(26,31,58,0.08)'}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}><div style={{width:44,height:44,borderRadius:22,background:V.surface,display:'flex',alignItems:'center',justifyContent:'center'}}><GearIcon size={22}/></div><div><h2 style={{margin:0,fontFamily:FD,fontSize:24,color:V.navy,fontWeight:600}}>Account Settings</h2><p style={{margin:0,fontFamily:F,fontSize:12,color:V.textLight}}>Manage your profile</p></div></div>
      <div style={{display:'flex',flexDirection:'column',gap:16}}>
        <Input label="Your Name" value={name} onChange={setName} placeholder="Your display name" required/>
        <Input label="WhatsApp Number" value={phone} onChange={val=>setPhone(val)} placeholder="e.g. +971 50 123 4567" required type="tel" note="Contact number for your pet listings. Never shown publicly."/>
        <Input label="Address" value={address} onChange={setAddress} placeholder="Optional — villa/unit number" note="Optional. Never shown publicly."/>
        <div style={{padding:'14px 16px',background:V.surface,borderRadius:10}}><p style={{margin:0,fontFamily:F,fontSize:14,color:V.navy}}><strong>Email:</strong> {user.email}</p><p style={{margin:'4px 0 0',fontFamily:F,fontSize:11,color:V.textLight}}>Email is used for login and cannot be changed here.</p></div>
        ...
        <div style={{borderTop:'1px solid '+V.border,paddingTop:20,marginTop:8}}>
          ...
          ):(<div style={{padding:14,background:'#fef2f2',borderRadius:12,display:'flex',flexDirection:'column',gap:10}}><p style={{margin:0,fontFamily:F,fontSize:13,color:V.danger,fontWeight:700}}>Are you sure? All pets will be removed.</p><div style={{display:'flex',gap:10}}><button type="button" onClick={onDelete} style={{flex:1,padding:'10px',borderRadius:8,background:V.danger,color:'white',border:'none',fontFamily:F,fontWeight:700,fontSize:13,cursor:'pointer'}}>Yes, Delete</button><button type="button" onClick={()=>setConfirmDel(false)} style={{flex:1,padding:'10px',borderRadius:8,background:'white',color:V.text,border:'1px solid '+V.border,fontFamily:F,fontSize:13,cursor:'pointer'}}>Cancel</button></div></div>)}
        </div>
      </div>
    </div>
  </div>);
...
function PetForm({initial,user,onSubmit,onCancel,submitLabel}){
  const[type,setType]=useState(initial?.type||'');const[breed,setBreed]=useState(initial?.breed||'');const[name,setName]=useState(initial?.name||'');const[description,setDescription]=useState(initial?.description||'');const[lifestyle,setLifestyle]=useState(initial?.lifestyle||'indoor_outdoor');const[photos,setPhotos]=useState(initial?.photo_urls||[]);const[errors,setErrors]=useState({});const[submitting,setSubmitting]=useState(false);
  ...
  const handleSubmit=async()=>{if(!validate()||submitting)return;setSubmitting(true);try{
    if(initial?.id){const{error}=await supabase.from('pets').update({type,breed,name:name.trim(),description:description.trim()||null,lifestyle,photo_urls:photos}).eq('id',initial.id);if(error)throw error;}
    else{const{error}=await supabase.from('pets').insert({type,breed,name:name.trim(),description:description.trim()||null,lifestyle,owner_id:user.id,whatsapp:user.phone,photo_urls:photos});if(error)throw error;}
  ...
  return(<div style={{animation:'fadeIn 0.3s ease'}}>
    ...
    <div style={{background:V.cardBg,borderRadius:16,padding:'32px 28px',border:'1px solid '+V.border,boxShadow:'0 8px 30px rgba(26,31,58,0.08)'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:18}}>
        ...
        <button type="button" onClick={handleSubmit} disabled={submitting} style={{width:'100%',padding:'15px',borderRadius:10,border:'none',background:submitting?V.textLight:'linear-gradient(135deg,'+V.navy+','+V.navyLight+')',color:'white',fontSize:16,fontWeight:700,fontFamily:F,cursor:submitting?'wait':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,boxShadow:'0 4px 16px rgba(26,31,58,0.2)',marginTop:4}}><PawIcon size={16} color={V.gold}/> {submitting?'Saving...':(submitLabel||'Register Pet')}</button>
      </div>
    </div>
  </div>);
}
...
function PetCard({pet,onClick}){
  ...
  return(<div onClick={onClick} style={{background:V.cardBg,borderRadius:14,overflow:'hidden',cursor:'pointer',border:'1px solid '+V.border,transition:'all 0.3s',boxShadow:'0 2px 12px rgba(26,31,58,0.06)'}}
    ...
    <div style={{position:'relative',paddingTop:'70%',background:'linear-gradient(135deg,'+V.navyLight+','+V.navy+')'}}>{photo?<img src={photo} alt={pet.name} style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>{isCat?<CatSvg size={60}/>:<DogSvg size={60}/>}</div>}
      ...
      <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(26,31,58,0.85))',padding:'20px 14px 10px'}}><p style={{margin:0,fontFamily:F,fontSize:11,color:'rgba(255,255,255,0.7)',fontWeight:600,textTransform:'uppercase',letterSpacing:0.5}}>{pet.breed}</p></div></div>
    <div style={{padding:'14px 16px 16px'}}><h3 style={{margin:0,fontSize:20,fontFamily:FD,color:V.navy,fontWeight:600}}>{pet.name}</h3>
      ...
      <div style={{display:'flex',gap:12,marginTop:12,paddingTop:12,borderTop:'1px solid '+V.border}}><div style={{display:'flex',alignItems:'center',gap:5}}><PawIcon size={14} color={V.gold}/><span style={{fontFamily:F,fontSize:12,color:V.textLight,fontWeight:600}}>{pet.breed.split(' ')[0]}</span></div>
        <div style={{display:'flex',alignItems:'center',gap:5}}><span style={{fontSize:12}}>{lifestyleLabel(pet.lifestyle).split(' ')[0]}</span><span style={{fontFamily:F,fontSize:12,color:V.textLight,fontWeight:600}}>{pet.lifestyle==='indoor'?'Indoor':pet.lifestyle==='outdoor'?'Outdoor':'In & Out'}</span></div></div></div>
  </div>);
}
...
function PetDetail({pet,isOwner,onBack,onDelete,onEdit}){
  ...
  const waLink=(pet.whatsapp_link||'#')+'?text='+encodeURIComponent('Hi! I saw your pet "'+pet.name+'" ('+pet.breed+') on PawPrint Villanova and wanted to reach out.');
    ...
    <div style={{background:V.cardBg,borderRadius:16,overflow:'hidden',border:'1px solid '+V.border,boxShadow:'0 8px 30px rgba(26,31,58,0.08)'}}>
      ...
      <div style={{padding:'24px 28px 28px'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,flexWrap:'wrap'}}>
          ...
          <span style={{background:V.surface,padding:'4px 10px',borderRadius:6,fontSize:12,fontFamily:F,fontWeight:700,color:V.navy}}>{lifestyleLabel(pet.lifestyle)}</span></div>
        ...
        {pet.lifestyle==='indoor'&&(<div style={{marginTop:16,padding:'10px 14px',background:'rgba(201,168,76,0.08)',borderRadius:10,border:'1px solid rgba(201,168,76,0.2)'}}><p style={{margin:0,fontFamily:F,fontSize:12,color:V.goldDark,fontWeight:700}}>🏠 Indoor pet — if spotted outside, they may need help getting home.</p></div>)}
        {!isOwner&&(<>
          <div style={{marginTop:20,padding:16,background:'rgba(45,138,110,0.05)',borderRadius:12,border:'1px solid rgba(45,138,110,0.12)',display:'flex',alignItems:'flex-start',gap:10}}>
            <span style={{color:V.accent,flexShrink:0,marginTop:1}}><ShieldIcon size={18}/></span>
            <div><p style={{margin:0,fontSize:13,fontFamily:F,color:V.accent,lineHeight:1.5,fontWeight:600}}>Privacy protected</p><p style={{margin:'4px 0 0',fontSize:12,fontFamily:F,color:V.accent,lineHeight:1.5}}>The owner's number is hidden. Tapping below opens WhatsApp with a pre-filled message — the owner's profile only becomes visible once they choose to reply to you.</p></div>
          </div>
          <a href={waLink} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,padding:'15px 28px',borderRadius:12,background:V.whatsapp,color:'white',textDecoration:'none',fontFamily:F,fontWeight:800,fontSize:16,marginTop:16,boxShadow:'0 4px 16px rgba(37,211,102,0.3)'}}><WAIcon size={20}/> Contact Owner via WhatsApp</a>
        </>)}
...
export default function App(){
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({data:{session}})=>{if(session?.user){const u=session.user;setUser({id:u.id,email:u.email||'',phone:u.user_metadata?.whatsapp||'',name:u.user_metadata?.display_name||'PawPrint User',address:u.user_metadata?.address||null});}setCheckingAuth(false);});
    const{data:{subscription}}=supabase.auth.onAuthStateChange((ev)=>{if(ev==='SIGNED_OUT')setUser(null);});
    ...
    <div style={{minHeight:'100vh',background:V.cream}}>
      <div style={{background:V.navy,padding:'0 20px',position:'sticky',top:0,zIndex:40}}>
        <div style={{maxWidth:640,margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'center',height:60}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}><PawIcon size={20} color={V.gold}/><div><span style={{fontFamily:FD,fontSize:18,color:'white',fontWeight:600,display:'block',lineHeight:1.1}}>PawPrint</span><span style={{fontFamily:F,fontSize:9,color:V.gold,fontWeight:800,textTransform:'uppercase',letterSpacing:2}}>Villanova</span></div></div>
      ...
      <div style={{maxWidth:640,margin:'0 auto',padding:'20px 16px 90px'}}>
        ...
        {view==='browse'&&page==='browse'&&(<div style={{animation:'fadeIn 0.3s ease'}}>
          <div style={{background:'linear-gradient(135deg,'+V.navy+' 0%,'+V.navyLight+' 100%)',borderRadius:16,padding:'28px 24px',marginBottom:20,position:'relative',overflow:'hidden'}}>
            ...
            <div style={{position:'relative'}}><h2 style={{margin:'0 0 4px',fontFamily:FD,fontSize:24,color:'white',fontWeight:600}}>Villanova Pet Registry</h2><p style={{margin:'0 0 16px',fontFamily:F,fontSize:13,color:'rgba(255,255,255,0.5)'}}>Browse registered pets in our community</p>
              <div style={{display:'flex',gap:10}}>{[{n:pets.length,l:'Registered',bg:'rgba(255,255,255,0.08)',c:'white'},{n:pets.filter(p=>p.type==='cat').length,l:'Cats',bg:'rgba(201,168,76,0.15)',c:V.goldLight},{n:pets.filter(p=>p.type==='dog').length,l:'Dogs',bg:'rgba(45,138,110,0.15)',c:'#7ddfbe'}].map((s,i)=>(<div key={i} style={{flex:1,background:s.bg,borderRadius:10,padding:'12px 10px',textAlign:'center'}}><div style={{fontSize:22,fontWeight:800,fontFamily:F,color:s.c}}>{s.n}</div><div style={{fontSize:10,color:'rgba(255,255,255,0.5)',fontFamily:F,fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>{s.l}</div></div>))}</div></div>
          </div>
          <div style={{display:'flex',gap:10,marginBottom:18,flexWrap:'wrap'}}>
            <div style={{flex:1,minWidth:160,position:'relative'}}><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or breed..." style={{width:'100%',padding:'11px 14px 11px 38px',borderRadius:10,border:'1.5px solid '+V.border,fontSize:14,fontFamily:F,background:'white',outline:'none',boxSizing:'border-box'}}/><span style={{position:'absolute',left:13,top:'50%',transform:'translateY(-50%)',fontSize:15,opacity:0.35}}>🔍</span></div>
            <div style={{display:'flex',gap:3,background:'white',borderRadius:10,padding:3,border:'1.5px solid '+V.border}}>{['all','cat','dog'].map(f=>(<button type="button" key={f} onClick={()=>setTab(f)} style={{padding:'8px 16px',borderRadius:8,border:'none',cursor:'pointer',background:tab===f?V.navy:'transparent',color:tab===f?'white':V.textLight,fontFamily:F,fontSize:13,fontWeight:700}}>{f==='all'?'All':f==='cat'?'🐱 Cats':'🐶 Dogs'}</button>))}</div>
          </div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}><h3 style={{margin:0,fontFamily:FD,fontSize:20,color:V.navy,fontWeight:600}}>Registered Pets</h3><span style={{fontFamily:F,fontSize:12,color:V.textLight,fontWeight:600}}>{filtered.length} found</span></div>
        ...
        {view==='browse'&&page==='myPets'&&(<div style={{animation:'fadeIn 0.3s ease'}}>
          ...
          {myPets.length>0?(<div style={{display:'flex',flexDirection:'column',gap:14}}>
            {myPets.map(pet=>(<div key={pet.id} style={{background:V.cardBg,borderRadius:14,border:'1px solid '+V.border,padding:16,display:'flex',gap:16,alignItems:'center',cursor:'pointer',transition:'all 0.2s',boxShadow:'0 2px 8px rgba(26,31,58,0.04)'}} onClick={()=>{setSelectedPet(pet);setView('detail');}}
              ...
              <div style={{width:75,height:75,borderRadius:10,overflow:'hidden',background:'linear-gradient(135deg,'+V.navyLight+','+V.navy+')',flexShrink:0}}>{(pet.photo_urls&&pet.photo_urls[0])?<img src={pet.photo_urls[0]} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center'}}>{pet.type==='cat'?<CatSvg size={38}/>:<DogSvg size={38}/>}</div>}</div>
              <div style={{flex:1,minWidth:0}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><h3 style={{margin:0,fontFamily:FD,fontSize:18,color:V.navy,fontWeight:600}}>{pet.name}</h3><span style={{background:pet.type==='cat'?V.gold:V.accent,color:'white',padding:'3px 10px',borderRadius:5,fontSize:10,fontWeight:800,textTransform:'uppercase',fontFamily:F}}>{pet.type}</span></div>
                <p style={{margin:'3px 0',fontSize:12,color:V.textLight,fontFamily:F,fontWeight:600}}>{pet.breed} · {lifestyleLabel(pet.lifestyle)}</p>
                <div style={{display:'flex',gap:8,marginTop:8}}><button type="button" onClick={e=>{e.stopPropagation();setEditPet(pet);setView('edit');}} style={{padding:'5px 14px',borderRadius:6,border:'1px solid '+V.border,background:'white',fontFamily:F,fontSize:11,fontWeight:700,color:V.navy,cursor:'pointer',display:'flex',alignItems:'center',gap:4}}><EditIcon/> Edit</button></div></div>
            </div>))}
