...
function Btn({ children, onClick, style, disabled, type }) {
  return <button type={type||'button'} onClick={onClick} disabled={disabled} style={{ border:'none', cursor: disabled?'default':'pointer', fontFamily:F, fontWeight:600, fontSize:14, borderRadius:10, padding:'10px 18px', transition:'all 0.15s', ...style }}>{children}</button>;
}
...
function PhotoUploader({ photos, setPhotos }) {
  ...
  const handleAdd = e => {
    Array.from(e.target.files).slice(0,3-photos.length).forEach(file => {
      ...
      r.onload = ev => setPhotos(p => p.length>=3?p:[...p, ev.target.result]);
...
function AuthScreen({ onLogin }) {
  ...
  const verifyOtp = () => {
    ...
    onLogin({ id: userId, phone: phone.trim(), name: name.trim() });
  };
          ...
          {step === 'phone' ? (
            <>
              <div style={{textAlign:'center',marginBottom:20}}>
                ...
                <h2 style={{margin:'0 0 4px',fontFamily:FD,fontSize:22,color:C.text}}>Sign in with WhatsApp</h2>
                <p style={{margin:0,fontFamily:F,fontSize:13,color:C.textMuted}}>We'll send a verification code to your number</p>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                ...
                <Btn onClick={sendOtp} disabled={sending} style={{width:'100%',background:C.whatsapp,color:'white',fontSize:16,padding:'14px 24px',borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',gap:8,opacity:sending?0.7:1}}>
                  <WAIcon size={18}/> {sending ? 'Sending code...' : 'Send Verification Code'}
                </Btn>
              </div>
...
function PetForm({ initial, user, onSubmit, onCancel, submitLabel }) {
  ...
  const [photos, setPhotos] = useState(initial?.photo_urls||initial?.photos||[]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const petId = initial?.id || (Date.now().toString(36) + Math.random().toString(36).slice(2,6));
      const photoUrls = [];
      for (let i = 0; i < photos.length; i++) {
        if (photos[i].startsWith('data:')) {
          ...
          const path = petId + '/' + i + '.' + ext;
          const { error: upErr } = await supabase.storage.from('pet-photos').upload(path, blob, { upsert: true, contentType: blob.type });
      ...
      if (initial?.id) {
        ...
        const { error } = await supabase.from('pets').insert({
          ...
          owner_id: user.id, whatsapp: user.phone, photo_urls: photoUrls,
        });
        if (error) throw error;
      }
      onSubmit();
          ...
          {isOwner && (
            <div style={{marginTop:20,padding:16,background:'#fef3c7',borderRadius:14,border:'1px solid #fde68a'}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                ...
                {!confirmDelete ? (
                  <Btn onClick={()=>setConfirmDelete(true)} style={{background:'transparent',color:C.danger,border:'1.5px solid '+C.danger,flex:1,justifyContent:'center'}}>
                    Remove
                  </Btn>
                ) : (
...
export default function App() {
  ...
  const handlePetSaved = () => {
    ...
    setView('browse');
    setEditPet(null);
    showToast(editPet ? 'Updated!' : 'Pet registered! 🎉');
  };
    ...
    <div>
      ...
      <div style={{minHeight:'100vh',background:C.bg}}>
        ...
        <div style={{display:'flex',background:'white',borderBottom:'1.5px solid '+C.border}}>
          {[{k:'browse',label:'🏠 Browse All'},{k:'myPets',label:'🐾 My Pets ('+myPets.length+')'}].map(t=>(
        ...
        <div style={{maxWidth:600,margin:'0 auto',padding:'16px 16px 80px'}}>
          ...
          {view === 'browse' && page === 'browse' && (
            <div style={{animation:'fadeIn 0.3s ease'}}>
              ...
              {loading ? (
                <div style={{textAlign:'center',padding:'40px 20px'}}><PawIcon size={40} color={C.primary}/><p style={{fontFamily:F,color:C.textMuted,fontSize:14,marginTop:12}}>Loading pets...</p></div>
              ) : filtered.length>0 ? (
                ...
                <div style={{textAlign:'center',padding:'48px 20px',background:'white',borderRadius:16,border:'1.5px solid '+C.border}}>
                  <CatSvg size={64}/>
                  <h3 style={{fontFamily:FD,color:C.text,marginBottom:4}}>{pets.length===0?'No pets registered yet':'No pets match your search'}</h3>
                  <p style={{fontFamily:F,color:C.textMuted,fontSize:14}}>{pets.length===0?'Be the first to register!':''}</p>
                </div>
              )}
            </div>
          )}
